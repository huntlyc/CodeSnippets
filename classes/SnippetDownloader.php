<?php
/**
 * Wee helper class for the downloads
 *
 **/
class SnippetDownloader{    
    ...
    /**
     * packages snippet into handy zip file and then starts the download
     * @param Array snippet The code snippet to download.
     * @return void     
     **/    
    public function createZip($snippet){        
        ...
        if ($fd = fopen ($zipname, "r")) {
            ...
            $downloadName = strtolower($snippetTitle);
            $downloadName = preg_replace('/\s+/', '_', $downloadName);
            $downloadName = preg_replace('/[\W]+/', '', $downloadName). ".zip";
